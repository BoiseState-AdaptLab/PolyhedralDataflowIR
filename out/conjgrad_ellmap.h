// 'conjgrad_ellmap' code generated by 'edavis' at 11/14/2019 13:05:52
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <math.h>
...
#define col(n) col[(n)]
#define A(n) A[(n)]
#define lmap(t,k,i) (int)(lmap[offset2((k),(i),N)])-1

double conjgrad_ellmap(const double* A, const double* b, const unsigned K, const unsigned N, const unsigned T, const unsigned* col, const unsigned* lmap, double* x);
inline double conjgrad_ellmap(const double* A, const double* b, const unsigned K, const unsigned N, const unsigned T, const unsigned* col, const unsigned* lmap, double* x) {
...
#undef s3
#define s3(t,k,i,n,j) s[(i)]+=A[(n)]*d[(j)]
#undef s4
...
for(t2 = 1; t2 <= T; t2++) {
  ...
  //for(t4 = 0; t4 <= N-1; t4++) {
    ...
    //for(t6 = 0; t6 <= K-1; t6++) {
      ...
      if (t8>=0) {
        t10 = col(t8);
        //fprintf(stderr,"t=%u,k=%u,i=%u,j=%u\n", t2,t6,t4,t8);
        s3(t2, t6, t4, t8, t10);
      }
    }
    //s4(t2,t4);
